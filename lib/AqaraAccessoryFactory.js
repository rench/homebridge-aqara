...
const commander = require('./Commanders');
...
AqaraAccessoryFactory.prototype.setGatewayLight = function (gatewaySid, deviceSid, rgbValue) {
  ...
  var hueColor = colors.rgb_to_hkhue(red, green, blue);
  var hue = hueColor.hue;
  var sat = hueColor.sat;
  ...
  this.log.debug("set getway light hue:" + JSON.stringify(hueColor));
  this.log.debug("set gateway light hue " + hue + ',sat:' + sat);
  ...
  var cmder = accessory.getService(Service.Lightbulb).getCharacteristic(Characteristic.On).listeners('set');
  if (cmder && cmder.length > 0) {
    cmder[0].lastValue = rgbValue;
  }
  ...
  this.log.debug("set gateway light hue to: " + hue)
  this.findServiceAndSetValue(
    gatewaySid,
    deviceSid,
    accessoryUUID,
...
AqaraAccessoryFactory.prototype.registerGatewayCommander = function (platform, gatewayAccessory) {
  ...
  var gatewayCommander = new commander.GatewayCommander(platform, deviceSid, "gateway", gatewayAccessory);
  service.getCharacteristic(Characteristic.Hue).on("set", function (value, callback) {
    platform.log.debug(util.format("set Hue to: %s", value));
    gatewayCommander.send(value);
    callback();
  })

  service.getCharacteristic(Characteristic.Saturation).on("set", function (value, callback) {
    platform.log.debug(util.format("set Saturation to: %s", value));
    gatewayCommander.send(null, value);
    callback();
  })
  ...
  service.getCharacteristic(Characteristic.Brightness).on("set", function (value, callback) {
    ...
    gatewayCommander.send(null, null, value);
    callback();
  })

  service.getCharacteristic(Characteristic.On).on("set", function (value, callback) {
    platform.log.debug(util.format("start gateway light to: %s", value));
    gatewayCommander.send(null, null, null, value ? 'on' : 'off');
    callback();
  })
